...
const App = () => {
  ...
  const renderView = () => {
    switch (currentView) {
      ...
      case 'PHARMACY_DASHBOARD':
        return <PharmacyDashboard user={user} onLogout={handleLogout} targetTab={pharmacyDashboardTab} />;
                ...
                <div className="relative w-20 h-20 flex items-center justify-center mb-6">
                    {/* Animated Rings */}
                     ...
                     <div className="bg-gradient-to-br from-emerald-500 to-teal-600 w-16 h-16 rounded-full flex items-center justify-center shadow-xl border border-white/20 relative z-10">
                         <Pill size={32} className="text-white -rotate-45 drop-shadow-md" strokeWidth={2} />
                     </div>
                     <div className="absolute -bottom-1 -right-1 bg-white text-emerald-600 rounded-full p-1.5 border-2 border-emerald-600 shadow-md z-20">
  ...
  return (
    <div className={`min-h-screen bg-gray-50 dark:bg-gray-950 dark:text-gray-100 pb-24 md:pb-8 font-cairo transition-colors duration-300`}>
      {showNavbar && <Navbar currentView={currentView} onNavigate={setCurrentView} user={user} onLogout={handleLogout} isDarkMode={isDarkMode} toggleTheme={toggleTheme} />}
